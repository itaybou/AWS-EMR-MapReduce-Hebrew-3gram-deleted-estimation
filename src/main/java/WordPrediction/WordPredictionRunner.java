...
public class WordPredictionRunner {
    ...
    public static void main(String[] args) throws IOException {
        ...
        final Job joinThreeGramNrTrThreeGrams = Job.getInstance(joinThreeGramNrTrThreeGramsConfig, "Join Nr Tr With 3Grams");
        String joinedThreeGramNrTrThreeGramsPath = createJoinThreeGramNrTrJob(joinThreeGramNrTrThreeGrams, splitCorpusPath, aggregatedNrPath, aggregatedTrPath);
        ...
        final Job calculateDeletedEstimationsJob = Job.getInstance(jobCalculateDeletedEstimationsConfig, "Calculate Deleted Estimations");
        String calculateDeletedEstimationsPath = createDeletedEstimationJob(calculateDeletedEstimationsJob, joinedThreeGramNrTrThreeGramsPath);
        waitForJobCompletion(calculateDeletedEstimationsJob, calculateDeletedEstimationsPath);

        // Sort deleted estimation by first two words from 3Gram and break ties with the deleted estimation probability value
        ...
        System.out.printf("\nFinished all jobs successfully: output can be found in s3 path: %s%n", String.format("%s/result", outputBucketPath));
    }

    private static String setInputOutput(Job job, String inputPath, boolean finished) throws IOException {
        FileInputFormat.addInputPath(job, new Path(inputPath));
        String outputPath = finished ? String.format("%s/result", outputBucketPath) : String.format("%s/jobs/%s", outputBucketPath, job.getJobName());
    ...
    private static String createJoinThreeGramNrTrJob(Job job, String splitCorpusPath, String aggregatedNrPath, String aggregatedTrPath) {
    ...
    private static String createDeletedEstimationJob(Job job, String joinThreeGramNrTrThreeGramsPath) throws IOException {
        job.setJarByClass(CalculateDeletedEstimations.class);
    ...
    private static String SortJobMaker(Job job, String deletedEstimationsPath)
            throws IOException {
        job.setJarByClass(SortDeletedEstimations.class);
        job.setMapperClass(SortDeletedEstimations.MapperClass.class);
        job.setPartitionerClass(SortDeletedEstimations.PartitionerClass.class);
    ...
    private static void waitForJobCompletion(final Job job, String outputPath) {
        ...
        try {
            ...
            System.err.printf(
                    "Exception caught! EXCEPTION: %s\nLogs in S3 bucket at %s.%n",
                    e.getMessage(), LOG_PATH);
            System.exit(1);
        }
    }
}
